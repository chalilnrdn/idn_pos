...
class _QrResultModalState extends State<QrResultModal> {
  // variabel untuk menyimpan status cetak
  late bool _printFinished;

  @override
  void initState() {
    super.initState();
    //awalnya anggap proses print blm selesai
    _printFinished = false;

    //jika mode memcetak atau printer nyala kita buat simulasi loading
    if (widget.isPrinting) {
      Future.delayed(Duration(seconds: 2), () {
        //cek kalo proses delay sesuai dengan waktu yang dbutuhkan printer ketika mencetak
        if (mounted) {
          setState(() {
            _printFinished = true; //ubah status jadi selese
          });
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.isPrinting) {
      ...
      statusText = 'Mencetak Struk Sisik...';
    } else {
      // kondisi 1 = print selesai
